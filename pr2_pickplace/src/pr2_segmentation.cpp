...
#include <pr2_pickplace/SegmentedClustersArray.h>
...
public:
  ...
  explicit segmentation(ros::NodeHandle nh) : m_nh(nh)  {
    ...
    m_clusterPub = m_nh.advertise<pr2_pickplace::SegmentedClustersArray> ("pr2_pickplace/pcl_clusters",1);

    #ifdef DEBUG // add in the publishers to look at the output through Rviz if debugging
    m_voxelPub = m_nh.advertise<sensor_msgs::PointCloud2> ("pr2_pickplace/pcl_voxel",1);
    m_outliersPub = m_nh.advertise<sensor_msgs::PointCloud2> ("pr2_pickplace/pcl_ouliers",1);
    m_passthroughPub = m_nh.advertise<sensor_msgs::PointCloud2> ("pr2_pickplace/pcl_passthrough",1);
    m_ransacPub = m_nh.advertise<sensor_msgs::PointCloud2> ("pr2_pickplace/pcl_ransac",1);
    m_passthrough2Pub = m_nh.advertise<sensor_msgs::PointCloud2> ("pr2_pickplace/pcl_passthrough2",1);
    m_coloredClustersPub = m_nh.advertise<sensor_msgs::PointCloud2> ("pr2_pickplace/color_clusters",1);
    #endif
...
void segmentation::cloud_cb (const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
  ...
  pr2_pickplace::SegmentedClustersArray CloudClusters;
