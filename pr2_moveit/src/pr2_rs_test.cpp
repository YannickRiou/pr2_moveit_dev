...
void PR2RsTest::clickCallback(const geometry_msgs::Pose& pose)
{
  ...
  right_arm_move_group_.setPoseTarget(target_pose);
  right_arm_move_group_.plan(my_plan);
  right_arm_move_group_.move();
...
void PR2RsTest::pickplace(moveit::planning_interface::MoveGroupInterface& move_group)
{
...
// TODO: Need to be changed according to new topics
void PR2RsTest::markerCallback(const visualization_msgs::MarkerConstPtr& marker, moveit::planning_interface::PlanningSceneInterface& plan_scene)
{

  // TODO: Subscribe to message from PDG (pose, id) and ask to Ontology what mesh for this id.

  // Add table as a fixed collision object.
  moveit_msgs::CollisionObject collisionObj;
  collisionObj.id = std::to_string(marker->id);
  ...
  {
    std::cout << "NEW MESH HERE" << std::endl;
    shape_msgs::Mesh mesh;
    shapes::ShapeMsg mesh_msg;

    shapes::Mesh* m = shapes::createMeshFromResource(marker->mesh_resource);
  ...
  plan_scene.addCollisionObjects(collision_objects_vector_);
}
...
/*
bool PR2RsTest::pickObj(pr2_moveit::pickObj::Request  &req, pr2_moveit::pickObj::Response &res)
{
  std::cout << "Req id is " << std::to_string(req.id) << std::endl;
  geometry_msgs::Pose posePick;
  shape_msgs::SolidPrimitive target_dimension;
  ...
  for(auto obj : collision_objects_vector_)
  {
    if(obj.id.find(std::to_string(req.id)) != std::string::npos)
    {
      // Verify if the object is a mesh
      if(obj.meshes.size() > 0)
      {
        posePick = obj.mesh_poses[0];
        std::cout << "Pose pick loaded" << std::endl;
      }
      else if (obj.primitives.size() > 0)
      {
        posePick = obj.primitive_poses[0];
        target_dimension = obj.primitives[0];
      }
    }
  }
  ...
  // Grasp pose
  grasps[0].grasp_pose.header.frame_id = "base_footprint";
  grasps[0].grasp_pose.pose.position.x = posePick.position.x-0.20;
  grasps[0].grasp_pose.pose.position.y = posePick.position.y;
  grasps[0].grasp_pose.pose.position.z = posePick.position.z;
  ...
  // Call pick to pick up the object using the grasps given
  res.error_code = left_arm_move_group_.pick(std::to_string(req.id), grasps);

  return true;
}

bool PR2RsTest::placeObj(pr2_moveit::placeObj::Request  &req, pr2_moveit::placeObj::Response &res)
{
  ...
  // Place pose
  place_location[0].place_pose.header.frame_id = "base_footprint";
  place_location[0].place_pose.pose.position.x = req.posePlace.position.x;
  place_location[0].place_pose.pose.position.y = req.posePlace.position.y;
  place_location[0].place_pose.pose.position.z = req.posePlace.position.z;
  ...
  // Poste-place end effector pose
  openGripper(place_location[0].post_place_posture);

  res.error_code = left_arm_move_group_.place(std::to_string(req.id), place_location);

  return true;
}
*/

std::vector<moveit_msgs::CollisionObject>& PR2RsTest::getCollisionObjVector()
{
...
*/
  ...
  ros::Subscriber marker_sub = n.subscribe<visualization_msgs::Marker>("/visualization_marker", 1000, boost::bind(&PR2RsTest::markerCallback,&pr2_rs,_1,boost::ref(planning_scene_interface)));
  ...
  shapes::Mesh* m = shapes::createMeshFromResource("file:/home/yriou/ws_pr2_moveitOnly/src/pr2_moveit/obj/green_cylinder.obj");
  ...
  obj_pose.position.x = 0.7;
  obj_pose.position.y = 0;
  obj_pose.position.z = 0.95;
  ...
  pr2_rs.getCollisionObjVector().push_back(co);
  */

  // Now, let's add the collision object into the world
  ROS_INFO("Added tableLaas into the world");
  ros::Duration(0.5).sleep();
  planning_scene_interface.addCollisionObjects(pr2_rs.getCollisionObjVector());
  /*
  std::cout << "**** Trying Pick" << std::endl;
  ros::Duration(2).sleep();
  marker_sub.shutdown();
  pr2_rs.pickplace(pr2_rs.getMoveGroupInterface("left_arm_move_group"));
  */
  ros::waitForShutdown();
  return 0;
}
