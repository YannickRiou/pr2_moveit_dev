...
void PR2RsTest::openGripper(trajectory_msgs::JointTrajectory& posture,std::string armUsed)
{
  if(armUsed == "left_arm")
  {
...
void PR2RsTest::closedGripper(trajectory_msgs::JointTrajectory& posture,std::string armUsed)
{
  if(armUsed == "left_arm")
  {
    posture.joint_names.resize(6);
...
void PR2RsTest::pickplace(moveit::planning_interface::MoveGroupInterface& move_group,std::vector<moveit_msgs::CollisionObject>& collision_object_vector)
{
  ...
  if (target_dimension.type == target_dimension.CYLINDER)
  ...
  else if (target_dimension.type == target_dimension.BOX)
    grasps[0].grasp_pose.pose.position.x = target_pose.position.x-(target_dimension.dimensions[shape_msgs::SolidPrimitive::BOX_X]/2.0)-0.15;
  ...
  if(target_pose.position.y > 0)
  {
    ...
    // +++++++++++++++++++++++++++++++++++
    openGripper(grasps[0].pre_grasp_posture,armUsed);

    // Setting posture of eef during grasp
    // +++++++++++++++++++++++++++++++++++
    closedGripper(grasps[0].grasp_posture,armUsed);

    // Set support surface as table1.
    left_arm_move_group.setSupportSurfaceName("tableLaas");
    error = left_arm_move_group.pick(id, grasps);

  }
  else if (target_pose.position.y < 0)
  {
    armUsed = "right_arm";
    // Setting posture of eef before grasp
    // +++++++++++++++++++++++++++++++++++
    ...
    // Set support surface as table1.
    right_arm_move_group.setSupportSurfaceName("tableLaas");
    error = right_arm_move_group.pick(id, grasps);
  }
  else
  {
    armUsed = "left_arm";
  ...
  if(error == moveit::planning_interface::MoveItErrorCode::SUCCESS)
  {
    ...
    place_location[0].place_pose.pose.position.z = target_pose.position.z+0.02;
    ...
    openGripper(place_location[0].post_place_posture,armUsed);

    if(armUsed == "left_arm")
    {
      // Call place to place the object using the pose given
      left_arm_move_group.place(id, place_location);
    }
    else if(armUsed == "right_arm")
    {
      // Call place to place the object using the pose given
      right_arm_move_group.place(id, place_location);
    }
  }
}
